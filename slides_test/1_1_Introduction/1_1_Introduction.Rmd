...
The topic of spatial statistics would require a focus on actual statistics rather than hands-on data wrangling. Therefore, we decided to skip the topic of spatial statistics. We won't take a closer look at:
- The concept of spatial dependence
  - its modeling using spatial weight matrices
  - analysis of spatial dependence (e.g., Moran's I)
- Spatial regression

--

Still, all of these topics can be approached using `R`, e.g., with the
- `spdep` package
- `spatstat` package
- `spatialEco` package
- and so much more
...
- don't hesitate to bring up your own solutions

---

## Thus, we cannot cover everything, unfortunately

Even for the packages we use, we cannot cover all of their functions. For example, the package we will use extensively, the `sf` package, has even more capabilities than we show for:
...
- Research focus on political and electoral geography
- (other) research interests:
...
.center[`https://github.com/StefanJuenger/gesis-workshop-geoR`]
...
*In practice, you shouldn't worry to much about CRS. Again, what matters is that they match.*
...
Grab one of you favorite hot or cold beverages or take a stretch break.

If you're back at your place early, you may want to play around with geometries on this site: https://thetruesize.com
...
Currently, there's a transition from the a bit aged standard of `PROJ.4` to `WKT` in many `R` packages.
- you'll get a lot of warnings (see next slide)
- you should definitely check what's causing the warning
...
Eventually, it's not as difficult to work with CRS in `R` as it may seem
...
There's no more on geospatial data and their individual quirks for the time being.

Let's learn about them as we learn about specific formats:
- vector data (soon)
- raster data (this afternoon)

It should not be surprising in light of the this course's content: `R` can serve as a full-blown Geographic Information System (GIS) for all these data.
...
During the course we will use plenty of different packages, but only a few are our main drivers (e.g., die `sf` package). Here's the list of packages
...
There's only one `R`. However, due to the enormous amount of additional packages, routines for similar purposes can shape up rather differenlty.

A prime example is `Tidyverse`
...
I (again, Stefan) heavily use namespace declarations in code: `::`. It's a way to safely call a function if there a the same function names in other functions, which are also loaded. Usually, you don't need that, but it's a transparent way to show from which package you call which function. 
...
knitr::include_graphics("./img/cute_cat.jpg")
```
]

.right-column[
.left[.small[`r icon::fontawesome("envelope")` [`cute.cat@gesis.org`](mailto:cute.cat@gesis.org)] </br>
.small[`r icon::fontawesome("twitter")` [`@CuteCat`](https://twitter.com/CuteCat)] </br>
.small[`r icon::fontawesome("github")` [`cute_cat`](https://github.com/cute_cat)] </br>
.small[`r icon::fontawesome("home")` [`https://www.cute-cat.org`](https://www.cute-cat.org)]] </br>
]