...
Short Check-In: How are you feeling in two words? [Click here]()
...
However, this command does not allow us to manipulate the maps in any ways.
But we already have the two most important ingredients to create a nice map:
...
There is a difference between a great and an amazing map.
And there are hundreds of really really bad maps out there.
What makes the difference?
.pull-left[
A Good Map:
- reduction to most important informations
...
As always, *RStudio*  offers several ways to map spatial data and the offered packages are various.
What is out there?
  - base R graphics package: `maps` and `mapdata`  
...
- Secondly, it offers not only a static, but a quick interactive feature.
...
By changing the mode, we get an interactive map!

.pull-left[
```{r first-map-int-disp, eval = F}
tmap_mode("view")
...
.pull-right[
```{r first-map-int, echo = FALSE, eval = FALSE}
...
The inner constructions of each building block of `tm_elements` are the same.
...
We can change the map self but we can also change the over all layout like title, frame, fonts, margins, legend
position, and more. 
There is another building block organizing basically everything surrounding our map.
...
[Exercise](https://stefanjuenger.github.io/gesis-workshop-geospatial-techniques-R/exercises/1_3_1_Basic_Maps_question_question.html)

[Solution](https://stefanjuenger.github.io/gesis-workshop-geospatial-techniques-R/exercises/1_3_1_Basic_Maps_question_solution.html)
...
One simple benefit of quick maps: You can explore your data easily and make spatial correlations visible.
In the previous months, German researchers and journalists alike claimed that in areas, where the Right-Wing Populist Party *AfD* is supported by voters, the rate of Covid-19 infections is especially high.
The argument supporting this hypothesis is that *AfD* supporters have more conspiracy beliefs and are less compliant to state rules.
...
If we place the Covid-19 map and the AfD map right next to each other, we can actually see some correlations.
(Even though there might be a lot of unobserved confounders...)
...
.pull-right[
```{r arranged-maps,  echo = F, warning = F, message = F}
tmap_arrange(covid_map, afd_map)
```
]
...
For example we can add a background layer of neighboring countries, add a layer of German cities or the borders of the German states for orientation.
...
In the best cases, maps are easy to understand and a good way to transport (scientific) messages.

In the worst cases, they simplify (spurious) correlations and draw a dramatic picture of the world.

Maps can shape narratives and they do not only depend on manipulation of the data they are based on.
Decisions on which projection you use (remember the `true size` projector?), the segment of the world you choose and the colors you add have a high influence.

As an example, check out this amazing blog post on mapping the Coronavirus responsible by [Kenneth Field](https://www.esri.com/arcgis-blog/products/product/mapping/mapping-coronavirus-responsibly/)
