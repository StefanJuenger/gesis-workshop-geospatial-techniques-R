...
In this exercise, we are going to re-use the results from the previous one. If you don't have them available, just fire up the code below.
...
# just to place it outside of the map frame.
```
...
If we were to draw the compass arrow in the upper right corner, what do you think would be some reasonable offset values for the bounding box? The values define the start and end point of the arrow, thus they have to be adjusted to be vertical in space.
```
...
Admittedly, drawing custom elements is a matter of fiddling around. We want to draw an arrow, and we didn't had this before. So here's a way to use our compass data and add it to an existing `ggplot`. Please feel free to adapt it to your liking.
...
```{block, opts.label = "clues"}
There's a lot of stuff happening on the map. Think about applying the `ggsn::blank()` function to remove all the clutter. Also, for the unrotated map you obviously don't need the `rotate_data()` function, right?
```
